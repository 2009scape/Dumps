...
{
  // check to see if we've already intialized wombat recently
  // this has been problematic on sites using HTML template files from other domains
  // see https://webarchive.jira.com/browse/ARI-5674
  if(WB_wombat_replayPrefix && WB_wombat_captureDatePart && WB_wombat_origHost) { return; }

  // skip re-inits for embeds on youtube.com, to improve url generation and relative url replay
  // see https://webarchive.jira.com/browse/ARI-6146
  var initiatingPage = WB_Get_InitiatingPage();
  if (initiatingPage && (initiatingPage.includes("%3A%2F%2Fwww.youtube.com%2F", 66)) && (origHost !== "www.youtube.com")) { return; }
...
function WB_Get_InitiatingPage() {
	let result = document.cookie.match("(^|[^;]+)\\s*" + 'wayback.initiatingpage' + "\\s*=\\s*([^;]+)")
